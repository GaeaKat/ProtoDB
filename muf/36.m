$def .broadcast broadcast-loop
$def .split \split
$def .pmatch \pmatch
$def .popn \popn
 
$libdef player-config
: player-config
var x
begin
{
" Alynna's say config menu"
{ " Editing: " me @ ansi_unparseobj }cat
" "
{ " [^YELLOW^1^NORMAL^] Say: You " me @ "_say/def/say" getpropstr dup not if pop "say" then "" "," subst ", \"<text>\"" }cat 1 parse_ansi
{ " [^YELLOW^2^NORMAL^] OSay: " me @ " " me @ "_say/def/osay" getpropstr dup not if pop "says" then "" "," subst ", \"<text>\"" }cat 1 parse_ansi
" "
" [^YELLOW^R^NORMAL^]eturn to player edit" 1 parse_ansi
" [^YELLOW^Q^NORMAL^]uit" 1 parse_ansi
}tell
read x !
x @ tolower case
 "1" smatch when
  { "What verb would you like to use when you see yourself talk (ex: yerf)" }tell
  { "ENTER resets this to the default, 'say':" }tell
  read_wants_blanks read x ! me @ "_say/def/say" x @ setprop
 end
 "2" smatch when
  { "What verb would you like to use when others see you talk (ex: yerf)" }tell
  { "ENTER resets this to the default, 'says':" }tell
  read_wants_blanks read x ! me @ "_say/def/osay" x @ setprop
 end
 "R" smatch when
  "$cmd/playeredit" match call
 end
 "Q" smatch when
  { "Terminated." }tell exit
 end
endcase
repeat
;
...
: get-pose-fmt (string fmt -- fmt')
   over if over 1 strcut pop else " " then
   " ,.:'!?;"
   swap instr if
      swap " " .split pop strlen 3 <
...
: rpose-fmt (s d -- s')
   "_rpose" getpropstr
   dup not if
      pop loc @ "_rpose" getpropstr
   then
   dup not if
      pop dup "In %l, %n %m"
   else over swap
   then
   "%n" "%N" subst "%m" "%M" subst "%l" "%L" subst
   dup "%m" instr not if " %m" strcat then
   dup "%n" instr not if "%n %m" "%m" subst then
   get-pose-fmt
   me @ name "%n" subst
   loc @ "%n" pronoun_sub "%l" subst
   swap "%m" subst
;
 
: broadcast-loop (str dbref -- )
   dup not if pop pop exit then
   dup "_broadcast?" getpropstr
   "yes" stringcmp not if          (str exitd)
      dup getlink dup ok? if       (str exitd linkd)
         dup room? if
            #-1 4 pick             (str exitd linkd #-1 str)
            4 pick rpose-fmt       (str exitd linkd #-1 str')
            notify_except
         else pop
         then
      else pop
...
dup mee @ dbcmp not if
        dup "_say/normal" getpropstr not if
          swap 1 + over
          over 4 + pick
          "^SAY/SAY^" swap "^NORMAL^" strcat strcat ansi_notify ( Do osay notify )
          over
    then then then
    next dup ok? not
  until
  ...
  loc @ over 3 + put
  dup 2 + rotate
  "^SAY/SAY^" swap "^NORMAL^" strcat strcat ansi_notify_exclude
;
: mix-for-notify ( quotes sayt osayt message -- 0 osay csay )
  ...
  -3 rotate over over strcmp if  ( sayt != osayt )
      -4 rotate 2 (say) mix-verb pop mee @ swap "^SAY/SAY^" swap "^NORMAL^" strcat strcat ansi_notify
      ...
      -4 rotate 1 (osay) mix-verb if (osay)
          dup mee @ swap "^SAY/SAY^" swap "^NORMAL^" strcat strcat ansi_notify
          -4 rotate rot (csay) mix-verb pop 2 put pop 0 exit
      else (csay)
          3 put pop pop me @ over "^SAY/SAY^" swap "^NORMAL^" strcat strcat ansi_notify dup 0 exit
  then then
;
...
;
: start-say ( message -- )
dup "HUH:*" smatch if ":" split swap pop strip then
